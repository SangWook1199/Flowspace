...
import { activities, calendar, kpis, members, todayTasks } from '../mock/dashboard';
...
export default function Dashboard() { return <div className={styles.content}><section className={styles.welcome}><div><h1>안녕하세요, 상욱님 <span>👋</span></h1><p>오늘도 FlowSpace와 함께 더 나은 협업을 만들어가요.</p></div><time>2025.05.28 (수)</time></section><div className={styles.dashboardGrid}><div className={styles.primary}><SprintBanner/><div className={styles.kpis}>{kpis.map(item => <MetricCard key={item.label} item={item}/>)}</div><div className={styles.lowerGrid}><TodayWorkCard tasks={todayTasks}/><div className={styles.midStack}><SprintStatusCard/><CalendarCard events={calendar}/></div></div></div><aside className={styles.rightRail}><ActivityCard activities={activities}/><OnlineTeamCard members={members}/></aside></div></div>; }
