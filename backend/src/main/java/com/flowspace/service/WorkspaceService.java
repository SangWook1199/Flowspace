...
public class WorkspaceService {
    ...
    public WorkspaceInviteResponse inviteMember(Long workspaceId, WorkspaceInviteRequest request, String email) {
        ...
        workspaceInviteRepository.findByWorkspaceAndEmailAndStatus(workspace, request.email(), InviteStatus.PENDING)
            .ifPresent(invite -> {
                throw new FlowSpaceException(ErrorCode.ALREADY_INVITED);
            });

        WorkspaceInvite invite = WorkspaceInvite.builder().workspace(workspace).inviter(inviter).email(request.email())
